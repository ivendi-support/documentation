...
To quote a new vehicle in [widget](./Widget.mdx), the following criteria will need to be adhered to. A `vrm` must be set to the value "NEW". Since there is no VRM for us to do a lookup on,
...
The `RegistrationDate` should also be today's date, rolling. See the below example Vehicle object extract of a [payment search](./PaymentSearch.mdx) request for an example.
