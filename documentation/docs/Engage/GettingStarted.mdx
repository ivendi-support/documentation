...
```html
 ...
 {
    id: "4caad104-3c7c-4716-9ae3-20174c0f10b8",
...
The FPA structure is somewhat flexible, in that any combination of static text and vehicle attribute variables can be used. For now, we are assuming that customers will only require make, model and id (unique vehicle id) as supplied in feed files. We will expand this to include other attributes from the feed files as and when required. If your website requires us to offer more variables from the feed file please do let us know. However, if the FPA URL structure requires dynamic content this should be handled via URL redirects from your end.
